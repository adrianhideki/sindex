        ...
        private void grdIndexes_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (grdIndexes.SelectedRows.Count > 0)
                {
                    ...
                    {
                        grdIndexes.SelectedRows[i].Cells[5].Value = !Boolean.Parse(grdIndexes.SelectedRows[i].Cells[5].Value.ToString());
                    }
                }
            } catch (Exception err)
            {
        ...
        private void chkMarcarTodos_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (grdIndexes.Rows.Count > 0)
                {
                    ...
                    {
                        grdIndexes.Rows[i].Cells[5].Value = chkMarcarTodos.Checked;
                    }
                }
            }
            catch (Exception err)
        ...
        private void criarToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (grdIndexes.Rows.Count > 0)
                {
                    for (int i = 0; i < grdIndexes.Rows.Count; i++)
                    {
                        if (Boolean.Parse(grdIndexes.Rows[i].Cells[5].Value.ToString()))
                        {
                            string database = grdIndexes.Rows[i].Cells[0].Value.ToString();

                            dbIndexes.CreateIndex(cred, database, grdIndexes.Rows[i].Cells[4].Value.ToString());
                        }
                    }
